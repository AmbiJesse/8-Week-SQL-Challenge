...
Recreate the following table which can be viewed [here](https://8weeksqlchallenge.com/case-study-1/)
```SQL
select
    s.customer_id,
    s.order_date,
    mn.product_name,
    mn.price,
    case when mb.customer_id is not null and order_date >= join_date then 'Y'
         else 'N' end as member

from dannys_diner.sales as s
left join dannys_diner.menu as mn using(product_id)
left join dannys_diner.members as mb using(customer_id)
...
Expanding the 'Join All The Things', Danny needs further information about the ranking of customer products, only for member purchases so null ranking values are expected when customers are not yet part of the loyalty program.
